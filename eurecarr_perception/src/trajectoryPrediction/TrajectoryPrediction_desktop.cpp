...
namespace eurecarr_perception {
  ...
  TrajectoryPrediction::TrajectoryPrediction(ros::NodeHandle nh)
  {
    ...
    nh_.param("speed_error", speed_error_, (float)1.0);
  ...
  void TrajectoryPrediction::openReferenceWaypoint(ros::NodeHandle nh)
  {
    ...
    std::string path_local = nh.param<std::string>("import_file", "/launch/waypoints");
...
//                   &GpsWaypoint::paramCallback, this);
  ...
  void TrajectoryPrediction::constructPrediction()
  {
    ...
    float speed_error = speed_error_;
    v = v * (1 + speed_error);
    ...
    while (travel < horizon){
      i += 1;
      float weight = pow(0.8, i);
      float next_x = (1-weight)*wptOneDim_re_.at(2*i) + weight*x0;
      float next_y = (1-weight)*wptOneDim_re_.at(2*i+1) + weight*y0;
      // float next_x = wptOneDim_re_.at(2*i) - wptOneDim_re_.at(0) + x0 + i*rv + prediction_mean_;
      // float next_y = wptOneDim_re_.at(2*i+1) -wptOneDim_re_.at(1) + y0 + i*rv + prediction_mean_;
  ...
  {
    // PREDICTION ERROR
    // TODO: noisy prediction
    // source: http://www.cplusplus.com/reference/random/normal_distribution/
    // std::normal_distribution<double> distribution_x(prediction_mean_x_, prediction_std_x_);
    // std::normal_distribution<double> distribution_y(prediction_mean_y_, prediction_std_y_);
    // rv_xt = distribution_x(generator_);
    //   rv_yt = distribution_y(generator_);
    double q0 = current_pose_.pose.pose.orientation.w;
    double q1 = current_pose_.pose.pose.orientation.x;
    double q2 = current_pose_.pose.pose.orientation.y;
    double q3 = current_pose_.pose.pose.orientation.z;
    double yaw = atan2(2*q1*q2 + 2*q0*q3, q0*q0 + q1*q1 + q2*q2 - q3*q3);
    double rv_xt = prediction_e_x_;
    double rv_yt = prediction_e_y_;
    double rv_x = 0;
    double rv_y = 0;
    rv_x = cos(yaw)*rv_xt - sin(yaw)*rv_yt;
    rv_y = sin(yaw)*rv_xt + cos(yaw)*rv_yt;
    ...
    int initial_i = 2;
    int k = 0;
    for (int i=initial_i; i<size; i+=2){
      ...
      for (int j=0; j<num_inter; j++){
        // inter_x = start_x + (float)(j)/(float)num_inter*(end_x - start_x) + (float(i-initial_i)/2.0*float(num_inter)+j)*rv_x;
        // inter_y = start_y + (float)(j)/(float)num_inter*(end_y - start_y) + (float(i-initial_i)/2.0*float(num_inter)+j)*rv_y;
        inter_x = start_x + (float)(j)/(float)num_inter*(end_x - start_x) + k*rv_x;
        inter_y = start_y + (float)(j)/(float)num_inter*(end_y - start_y) + k*rv_y;
...
**/
  ...
  void TrajectoryPrediction::ConfigCallback(const TrajectoryPredictionParamsConfig &config, uint32_t level)
  {
    ...
    speed_error_ = config.speed_error;
    prediction_e_x_ = config.prediction_error_x;
    prediction_e_y_ = config.prediction_error_y;
    // prediction_mean_x_ = config.prediction_mean_x;
    // prediction_mean_y_ = config.prediction_mean_y;
    // prediction_std_x_ = config.prediction_std_x;
    // prediction_std_y_ = config.prediction_std_y;
    hasNewTrajectoryParams_ = true;
    std::cout << "Got a config!!" << " (error_x per step, error_y per step) = (" << prediction_e_x_ << ", " << prediction_e_y_ << ")" << std::endl;
  }
